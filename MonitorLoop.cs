        ...
        return new ValueTask(Task.Factory.StartNew(() =>
        {
            try
            {
                    ...
                    props.Headers = new Dictionary<string, object>
                {
                    ...
                    channel.QueueBind("queue", "my", "queue");
                    channel.BasicPublish(exchange: "my", routingKey: "queue", basicProperties: props, body: Encoding.UTF8.GetBytes(Convert.ToBase64String(Encoding.UTF8.GetBytes($"{{\"guid\": \"{a.guid}\", \"preset\": \"{a.preset}\", \"source\": \"{a.source}\", \"destination\": \"{a.destination}\"}}"))));
                });
