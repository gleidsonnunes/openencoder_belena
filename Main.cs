                                   ...
                                   try
                                   {
                                       ...
                                       using (IModel channel = new ConnectionFactory { Uri = new("amqp://gleidson:Gleidson@gleidson-nunes.ddns.net/") }.CreateConnection().CreateModel())
                                       {
                                           jobs.ForEach(a =>
                                           {
                                               ...
                                               channel.BasicPublish(exchange: null, routingKey: "queue", basicProperties: props, body: Encoding.UTF8.GetBytes(Convert.ToBase64String(Encoding.UTF8.GetBytes($"{{\"guid\": \"{a.guid}\", \"preset\": \"{a.preset}\", \"source\": \"{a.source}\", \"destination\": \"{a.destination}\"}}"))));
                                           });
